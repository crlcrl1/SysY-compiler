...
（这部分无特别限制。这里给了一个参考书写建议，可根据需要适当粘贴少量代码。纯文字建议不要超过500字）
本编译器最核心的数据结构是……。在实现过程中，设计了数据结构xx用来……

```c
Sample code goes here
```

如果将一个SysY程序视作一棵树，如果将一个SysY程序视作一棵树，那么一个`struct CompUnit`的实例就是这棵树的根，根据这一情况设计了数据结构yy……
那么一个`struct CompUnit`的实例就是这棵树的根，根据这一情况设计了数据结构yy……
...
1. `lalrpop`：（100字以内，说明利用这个工具/模块/库做了什么）
2. `koopa`：（同上）
